...
4. **`ST87M01Client`** — Arduino `Client` over a TCP socket. `connect(const char*)` resolves DNS automatically. Uses the modem's URC-driven RX indication so `AT#IPREAD` is only issued when bytes are actually waiting. `AT#IPREAD` returns one IP frame per call (≤ `AT#IPPARAMS max_ip_frame_size`, default 1500 bytes); `read()` loops across successive `#IPRECV` URCs so multi-frame responses drain transparently.
...
- **`ST87M01HTTP`** — HTTP and HTTPS client built on the modem's native `AT#HTTP*` command family. Preferred over rolling your own HTTP on `ST87M01Client` because the modem also parses status lines, headers, and `Transfer-Encoding: chunked` for you — not because of any size limit (raw `ST87M01Client` now drains multi-frame responses transparently). For HTTPS, pass a TLS profile id (provisioned via `ST87M01TLS`) as the third arg to `begin()`.
...
- **Raw TCP RX is per-frame, but multi-frame is handled.** `AT#IPREAD` returns one IP frame per call (≤ `AT#IPPARAMS max_ip_frame_size`; ST documents the range as 512..1600 bytes, the library configures 1514 and the modem empirically delivers up to ~1540). Multi-frame responses arrive as successive `#IPRECV` URCs and are drained automatically by `ST87M01Client::read()` — arbitrary-sized responses work end-to-end (hardware-validated 2026-05-11 against `httpbin.org/bytes/4096` over 1NCE NB-IoT). The internal RX buffer is sized to the 1600-byte documented ceiling, so `client.droppedBytes()` should stay at zero under normal operation. `ST87M01HTTP` is still the right tool *for HTTP* because the modem additionally parses status lines, headers, and `Transfer-Encoding: chunked` — not because raw TCP can't handle the size.
